...
module tb_dea8_projection_engine;
  ...
  // Disjoint pre-edge occupancy buckets; their sum equals accept-to-done time.
  // Gap totals are separate diagnostics and must NOT be added a second time.
  always @(posedge clk) begin : performance_monitor
    if(!rst_n || clear) perf_tracking=0;
    else if(job_valid && job_ready) begin
      perf_tracking=1;perf_seen=0;perf_final_pending=0;
      perf_total=0;perf_issue=0;perf_cold=0;perf_a_wait=0;perf_w_wait=0;
      perf_drain=0;perf_pair_drain=0;perf_post=0;perf_control=0;
      perf_pair_gap=0;perf_tile_gap=0;perf_prev_cycle=0;perf_prev_nt=0;perf_prev_kt=0;
    end else if(perf_tracking) begin
      perf_total++;
      if(dut.req_valid && dut.req_ready) begin
        if(perf_seen && dut.req_tag.nt==perf_prev_nt && dut.req_tag.kt!=perf_prev_kt) begin
          perf_tile_gap+=perf_total-perf_prev_cycle-1;
          if(!dut.req_tag.kt[0]) perf_pair_gap+=perf_total-perf_prev_cycle-1;
        end
        perf_issue++;perf_seen=1;perf_prev_cycle=perf_total;
        perf_prev_nt=dut.req_tag.nt;perf_prev_kt=dut.req_tag.kt;
        if(dut.req_tag.final_k && dut.req_tag.row==SUFFIX_LEN-1) perf_final_pending=1;
      end else if(!perf_seen) perf_cold++;
      else if(dut.state_q==dut.POST_CMD || dut.state_q==dut.POST_WAIT) perf_post++;
      else if(perf_final_pending) perf_drain++;
      else if(dut.state_q==dut.PAIR_RX && !dut.pair_full) perf_a_wait++;
      else if(dut.state_q==dut.RUN_PAIR && !dut.pair_issue_done && dut.row_q==0 && !dut.tile_available) perf_w_wait++;
      else if(dut.state_q==dut.RUN_PAIR && dut.pair_issue_done) perf_pair_drain++;
      else perf_control++;
      if(commit_valid && commit_tag.final_k && commit_tag.row==SUFFIX_LEN-1) perf_final_pending=0;
      #1;if(job_done_valid) perf_tracking=0;
    end
  end

  task automatic report_performance;
    ...
    $fdisplay(fd,"perf_w_wait,0,%0d",perf_w_wait);
    $fdisplay(fd,"perf_final_drain,0,%0d",perf_drain);
    $fdisplay(fd,"perf_pair_drain,0,%0d",perf_pair_drain);
    $fdisplay(fd,"perf_post,0,%0d",perf_post);
    $fdisplay(fd,"perf_control,0,%0d",perf_control);
    $fdisplay(fd,"perf_pair_gap,0,%0d",perf_pair_gap);
    $fdisplay(fd,"perf_tile_gap,0,%0d",perf_tile_gap);
  endtask
