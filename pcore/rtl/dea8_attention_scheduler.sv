...
module dea8_attention_scheduler (
  ...
  always_comb begin
    ...
    matrix_cmd.ctx = '{block_id:b_q, head:head_q, epoch:epoch_q};
    matrix_cmd.op = (phase == RUN_PV || phase == TAIL_PV) ? MATRIX_PV : MATRIX_QK;
    if (phase == NEXT_QK) matrix_cmd.ctx.block_id = b_q + 1'b1;
    matrix_cmd.facc_bank = matrix_cmd.ctx.block_id[0];
    matrix_cmd.pbuf_bank = matrix_cmd.ctx.block_id[0];
    matrix_cmd.init_oacc = matrix_cmd.op == MATRIX_PV && b_q == 0;
    ...
    matrix_valid = rst_n && !matrix_sent &&
    ...
    advance = 0;
    case (phase)
      FIRST_QK, TAIL_PV: advance = matrix_finished;
      NEXT_QK: advance = matrix_finished && (b_q == 0 ? post == POST_DONE : vpu_finished);
      RUN_PV: advance = matrix_finished && post == POST_DONE;
      TAIL_SCALE, FINALIZE: advance = vpu_finished;
  ...
  initial if (N_KV_BLOCK < 2) $fatal(1, "Scheduler requires at least two KV blocks");
