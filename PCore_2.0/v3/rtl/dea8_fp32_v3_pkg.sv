package dea8_fp32_v3_pkg;
  ...
  function automatic logic [32:0] round_right32(input logic [31:0] value,input integer shift);
    logic [32:0] kept; logic guard_bit,sticky_bit;
  ...
  function automatic logic [31:0] pack_scaled32(
    input logic sign_bit,input logic [31:0] normalized,input integer unbiased,
    input logic is_zero,input logic bad_scale);
    logic [32:0] rounded; integer exponent_value; logic [7:0] exponent_field;
  ...
  function automatic logic [27:0] shift_right_jam28(input logic [27:0] value,input integer distance);
    logic [27:0] shifted; logic sticky_bit;
    shifted=distance>=28 ? 28'b0 : value>>distance; sticky_bit=0;
    for(int i=0;i<28;i++) if(i<distance) sticky_bit|=value[i];
    shifted[0]|=sticky_bit; return shifted;
  endfunction

  function automatic logic [31:0] fp32_add(input logic [31:0] a,b);
    logic [23:0] ma,mb,big_m,small_m; logic [27:0] big_x,small_x,sum_x;
    logic [24:0] rounded; logic sign_result,a_nan,b_nan,a_inf,b_inf;
    ...
    if(a_inf) return a; if(b_inf) return b;
    ea=a[30:23]==0?1:int'(a[30:23]); eb=b[30:23]==0?1:int'(b[30:23]);
    ...
    big_x={1'b0,big_m,3'b0}; small_x=shift_right_jam28({1'b0,small_m,3'b0},big_e-small_e);
    result_e=big_e; sum_x=a[31]==b[31]?big_x+small_x:big_x-small_x;
    if(sum_x==0) return {(a[31]&&b[31]),31'b0};
    if(sum_x[27]) begin sum_x=shift_right_jam28(sum_x,1); result_e++; end
    else begin
      top_bit=0; for(int i=0;i<27;i++) if(sum_x[i]) top_bit=i;
      normalize_shift=26-top_bit; if(normalize_shift>result_e-1) normalize_shift=result_e-1;
      sum_x=sum_x<<normalize_shift; result_e-=normalize_shift;
    end
